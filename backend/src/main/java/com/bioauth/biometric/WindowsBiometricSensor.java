...
public class WindowsBiometricSensor implements FingerprintSensor {
    ...
    public void initialize() throws SensorException {
        ...
        try {
            ...
            // Handle JNA linking errors specifically
            throw new SensorException("Failed to link with Windows Biometric Framework. " +
    ...
    private Pointer openBiometricSession() throws SensorException {
        try {
            ...
            if (WinBioAPI.WinBio.INSTANCE == null) {
                throw new SensorException("Windows Biometric Framework API not loaded");
            }
            ...
            if (result == WinBioAPI.WINBIO_SUCCESS) {
                ...
                throw new SensorException("Failed to open biometric session: " + errorMsg + " (Code: " + result + ")");
            }
        } catch (SensorException e) {
            throw e;
        } catch (UnsatisfiedLinkError e) {
            throw new SensorException("Failed to link with Windows Biometric Framework API: " + e.getMessage());
        } catch (Exception e) {
